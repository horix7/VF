  ...
  return (
    <div className={classes.root}>
          ...
          <div className={classes.demo}>
              ...
              {localCart.map((elem: any, key: Key) => (
                <ListItem key={key}>
                  ...
                  <ListItemText
                   ...
                   secondary={(<div className={classes.secondary}> {localStorage.currency} &nbsp; { Number(elem.price) * Number(localStorage.rate) }  &nbsp;  <span className="multiper"> <Icon iconName="CalculatorMultiply"/> </span>  &nbsp;   { Number(elem.amount || 1 )}</div>)}
                  />
            ...
            <div className="total">
              ToTal Amount : <span className="total_price"> {localStorage.currency }&nbsp; { localCart.length >= 1 ?  Number(localCart.map((elem: any) =>{
                if(elem.amount) {
                  return Number(elem.price) * Number(elem.amount)
                } else return Number(elem.price)
              }).reduce((a: number,b: number) => a + b )) * Number(localStorage.rate) : 0 }</span>
            </div>
          </div>
    </div>
  );
